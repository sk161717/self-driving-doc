...
### 編集してはいけないスクリプト
以下に挙げるスクリプトは，最終日の対戦時のプログラムと互換性がなくなるため，編集せずにそのまま使用してください．
（編集しても構いませんが，最終日の対戦時には配布しているプログラムを使用することを念頭に置いておいてください．）

AIの学習時などに既存スクリプトの挙動を変更したいときは，既存クラスを適切に継承するかコピペするかなどして，新しいクラスを作成するのがいいと思います．

- `AI/Battle/BattleBrain.cs`
- `AI/Battle/{NN, Q, PyNE}BattleBrain.cs`
- `Car/CarAgent.cs`, `Car/CarController.cs`
- `AI/AgentExecutor.cs`
